...
- **Deadline: 2026-09-19, 11:59 PM.** Phases 1–6 (all server work) are done on the night of 2026-09-18. Phases 7–11 follow on 2026-09-19.
- If a nice-to-have competes with finishing a phase, finish the phase.
- Each phase is planned, approved, implemented, then checked. Lint, typecheck and tests must pass, and `README.md` and `docs/API.md` are updated before handing over for review.
...
| `GET /analytics/summary`           | JWT              | Filters                                                                                                                                           | `{ totals: { revenue, expense, net, pending, pendingCount, count }, byCategory: [{ key, total, count }], byStatus: [{ key, revenue, expense, count }], byUser: [same shape as byStatus] }` |
| `GET /analytics/trend`             | JWT              | Filters + `interval=month\|week`                                                                                                                  | `{ interval, points: [{ period: ISODate, revenue, expense }] }`. Empty periods are filled with 0                                                                                           |
...
| · OverviewChart: smooth green/yellow lines, Monthly/Weekly select                                                                                                                                                | `GET /analytics/trend`                                                       |
...
| · BreakdownSection: category donut, status split, per-user revenue vs expense bars                                                                                                                               | `GET /analytics/summary` (same cached query)                                 |
...
| **ExportDialog**: column picker (select all/none, reorder with arrows), scope toggle (Current filters / All transactions), preview, row count, filename preview                                                  | `GET /transactions?pageSize=3` (preview), `POST /transactions/export` (blob) |
...
On small screens the sidebar becomes a drawer and the table scrolls sideways with a sticky first column.
...
- **Server:**
  - Unit tests cover the pure functions: `buildTransactionQuery`, `escapeRegex`, the error handler and CSV row mapping.
  - Integration tests use Supertest on `mongodb-memory-server`. The fallback is `MONGODB_TEST_URI`, as described under working rules.
...
| 2   | Seed                | Models and indexes; `seed.ts` validates each row with zod, upserts by `id`, runs `syncIndexes`, upserts the demo user; `--reset` flag; index reasoning in README                                                                   | Seed prints 300 / 339,803.25 / 206,605.00 / 205,303.00 (114) and exits non-zero on a mismatch; running it again changes nothing              |
...
| 5   | Analytics API       | Summary (`$facet`), trend (gap-filled)                                                                                                                                                                                             | Unfiltered totals match the seed; a filtered `count` equals the list `total` for the same filters                                            |
| 6   | Export API          | Column whitelist, streaming, headers                                                                                                                                                                                               | Tests cover header order and labels, filename, row count matching the list total, a bad column returning 400, and formula escaping           |
...
| 10  | Export modal        | ExportDialog, column picker, scope-aware preview, blob download, Blob error parsing                                                                                                                                                | Client test: Export is disabled with 0 columns; manual check that the file downloads with the right filename                                 |
...
**Stretch (only after 11):**

- Clicking a breakdown slice applies that filter.
- The top-bar search routes to `/transactions?search=`.
- Export column presets.
- The export dialog remembers the last column selection.

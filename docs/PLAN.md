...
- **Summary:** one `$group` by category and status (at most 4 buckets), so it's a single round trip. Every total is derived from those buckets.
...
| 5   | Analytics API       | Summary (one `$group`), monthly trend (gap-filled)                                                                                                                                                                                 | Unit tests: pipeline built from filters, totals rounded, empty months filled with 0                                                          |
